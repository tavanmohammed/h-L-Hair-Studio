...
function fmtLocalYMD(d = new Date()) {
...
function weekDays(weekStartYmd) {
  const out = [];
  for (let i = 0; i < 7; i++) out.push(addDays(weekStartYmd, i));
  return out;
}
...
function monthGrid(baseYmd) {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    cells.push({ ymd: fmtLocalYMD(d), inMonth: d.getMonth() === month });
  }
  return cells;
}
...
function dayRuleSummary(rules, ymd) {
  ...
  const isClosed = dayRules.some(r => r.kind === "closed");
  const hours = dayRules.find(r => r.kind === "hours");
  const blocks = dayRules.find(r => r.kind === "blocks");
  return { isClosed, hours, blocks, dayRules };
}

/* =========================
   Admin (mobile/tablet first)
========================= */
export default function Admin() {
  ...
  // dates
  const today = useMemo(() => fmtLocalYMD(new Date()), []);
  const tomorrow = useMemo(() => addDays(today, 1), [today]);
  const [selectedDay, setSelectedDay] = useState(today);

  // week scroller anchor
  const [weekStart, setWeekStart] = useState(startOfWeekYMD(today));

  // month grid anchor (for full calendar)
  ...
  async function loadMonthBookings(anchorYmd) {
    try {
      ...
      setBookingsErr("");
      const monthFirst = monthStartYMD(anchorYmd);
      const rows = await authFetch(`/api/admin/bookings?from=${monthFirst}`);
      const monthEnd = endOfMonthYMD(anchorYmd);
      setBookings(rows.filter(b => b.date >= monthFirst && b.date <= monthEnd));
    } catch (e) {
  ...
  async function loadMonthRules(anchorYmd) {
    try {
      ...
      const monthFirst = monthStartYMD(anchorYmd);
      const monthEnd = endOfMonthYMD(anchorYmd);
      const rows = await authFetch(`/api/admin/rules?from=${monthFirst}`);
      setRules(rows.filter(r => r.date >= monthFirst && r.date <= monthEnd));
    } catch (e) {
  ...
  async function saveHours() {
    try {
      setHoursMsg("");
      await authFetch("/api/admin/hours", { method: "PUT", body: JSON.stringify(hours) });
      setHoursMsg("Saved ✔");
  ...
  async function createBooking() {
    try {
      setQMsg("");
      if (!qName || !qPhone || !qEmail || !qServiceId || !qDate || !qTime) {
        setQMsg("Please fill all fields.");
        return;
      }
  ...
  async function addRule() {
    try {
      ...
      const saved = await authFetch("/api/admin/rules", { method: "POST", body: JSON.stringify(body) });
  ...
  // quick rule helpers
  async function quickCloseSelectedDay() {
    setRuleDate(selectedDay); setRuleKind("closed");
    await addRule(); await loadMonthRules(monthAnchor);
  }
  async function quickShortHoursSelectedDay() {
    setRuleDate(selectedDay); setRuleKind("hours"); setRuleOpen("12:00"); setRuleClose("16:00");
    await addRule(); await loadMonthRules(monthAnchor);
  }
  async function quickLunchBlockSelectedDay() {
    setRuleDate(selectedDay); setRuleKind("blocks"); setRuleBlocks([{ start: "13:00", end: "14:00" }]);
    await addRule(); await loadMonthRules(monthAnchor);
  }
        ...
        {rows.map(b => (
          <div key={b._id} className="rounded-xl border bg-white p-3 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="font-medium">{b.time}–{b.endTime}</div>
              <div className="text-xs px-2 py-0.5 rounded-full bg-gray-900 text-white">
                {b.serviceCategory}
              </div>
            </div>
  ...
  function BookingTable({ rows }) {
    if (!rows || rows.length === 0) return <div className="text-sm text-gray-500">No bookings.</div>;
    return (
  ...
  function DayChip({ ymd }) {
    ...
    return (
      <button
        ...
        className={`shrink-0 px-3 py-2 rounded-lg border text-sm mr-2 ${isSel ? "bg-black text-white border-black" : "bg-white"}`}
      >
  ...
  function DayCell({ cell }) {
    ...
    return (
      <button
        ...
        className={[
          "h-16 sm:h-20 border p-2 text-left relative transition",
      ...
      >
        <div className="text-xs">{cell.ymd.slice(8,10)}</div>
        {isToday && (
          <span className="absolute top-1 right-1 text-[10px] px-1 rounded bg-black text-white">today</span>
        )}
        ...
        {count > 0 && (
          <span className="absolute bottom-1 right-1 text-[11px] px-2 py-0.5 rounded-full bg-gray-900 text-white">
            {count}
          </span>
        )}
      </button>
  ...
  return (
    <div className="min-h-dvh bg-gray-50">
      ...
      <div className="p-4 sm:p-6 space-y-6">
        {/* Today & Tomorrow (cards on mobile) */}
        <details open className="bg-white rounded-xl shadow">
          <summary className="cursor-pointer select-none p-4 text-base sm:text-lg font-semibold">Today & Tomorrow</summary>
          <div className="p-4 pt-0 space-y-4">
            <div>
              <div className="text-sm text-gray-600 mb-1">Today ({today})</div>
              <div className="md:hidden"><BookingCards rows={todaysList} /></div>
              <div className="hidden md:block"><BookingTable rows={todaysList} /></div>
            </div>
            <div>
              <div className="text-sm text-gray-600 mb-1">Tomorrow ({tomorrow})</div>
        ...
        {/* Mobile Week scroller + optional Month grid */}
        <details open className="bg-white rounded-xl shadow">
          <summary className="cursor-pointer select-none p-4 text-base sm:text-lg font-semibold">
          ...
          <div className="p-4 pt-0 space-y-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="border px-3 py-2 rounded-lg active:scale-[.98]"
              >‹ Prev</button>
              ...
              <button
                onClick={() => setWeekStart(addDays(weekStart, 7))}
                className="border px-3 py-2 rounded-lg active:scale-[.98] ml-auto"
              >Next ›</button>
            </div>

            <div className="overflow-x-auto -mx-1 px-1">
              <div className="flex pb-1">
                {days.map(d => <DayChip key={d} ymd={d} />)}
              </div>
            </div>
            ...
            <button
              ...
              className="w-full border px-3 py-2 rounded-lg active:scale-[.98]"
            >
            ...
            {showMonth && (
              <div className="border rounded-lg p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="font-medium">{thisMonthLabel}</div>
                ...
                <div className="grid grid-cols-7 text-[11px] sm:text-xs text-gray-600 mb-1 px-1">
            ...
            <div className="grid sm:grid-cols-3 gap-2">
              <button onClick={quickCloseSelectedDay} className="border px-3 py-2 rounded-lg hover:bg-gray-50 active:scale-[.98]">
                Close day
              </button>
              <button onClick={quickShortHoursSelectedDay} className="border px-3 py-2 rounded-lg hover:bg-gray-50 active:scale-[.98]">
                12:00–16:00 hours
              </button>
              <button onClick={quickLunchBlockSelectedDay} className="border px-3 py-2 rounded-lg hover:bg-gray-50 active:scale-[.98]">
                Lunch 13:00–14:00
              </button>
            </div>
          </div>
        ...
        <details className="bg-white rounded-xl shadow">
          ...
          <div className="p-4 pt-0 space-y-4">
            <div className="grid md:grid-cols-3 gap-3">
              <input type="date" className="border rounded-lg p-3" value={ruleDate} onChange={e=>setRuleDate(e.target.value)} />
              <select className="border rounded-lg p-3" value={ruleKind} onChange={e=>setRuleKind(e.target.value)}>
              ...
              {ruleKind === "hours" && (
                <div className="flex gap-2">
                  <input type="time" className="border rounded-lg p-3 w-32" value={ruleOpen} onChange={e=>setRuleOpen(e.target.value)} />
                  <span className="self-center">to</span>
                  <input type="time" className="border rounded-lg p-3 w-32" value={ruleClose} onChange={e=>setRuleClose(e.target.value)} />
                </div>
              )}
                  ...
                  <div className="space-y-2">
                    {ruleBlocks.map((b, i) => (
                      <div key={i} className="flex gap-2">
                        <input type="time" className="border rounded-lg p-3 w-32" value={b.start}
                          onChange={e=>{ const v=[...ruleBlocks]; v[i]={...v[i], start:e.target.value}; setRuleBlocks(v); }} />
                        <span className="self-center">to</span>
                        <input type="time" className="border rounded-lg p-3 w-32" value={b.end}
                          onChange={e=>{ const v=[...ruleBlocks]; v[i]={...v[i], end:e.target.value}; setRuleBlocks(v); }} />
                        <button className="border px-3 py-2 rounded-lg active:scale-[.98]"
                          onClick={()=>{ const v=[...ruleBlocks]; v.splice(i,1); setRuleBlocks(v.length ? v : [{start:"13:00",end:"14:00"}]); }}>
                          Remove
                        </button>
                      </div>
                    ))}
                    <button className="border px-3 py-2 rounded-lg active:scale-[.98]" onClick={()=>setRuleBlocks([...ruleBlocks,{start:"13:00",end:"14:00"}])}>
                      + Add block
                    </button>
                  </div>
                </div>
              ...
              <div className="md:col-span-3 flex gap-2 items-center">
                <button onClick={async () => { await addRule(); await loadMonthRules(monthAnchor); }} className="bg-black text-white px-4 py-2 rounded-lg active:scale-[.98]">
                  Save rule
                </button>
          ...
          <div className="p-4 pt-0 space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="border rounded-lg p-3">
                ...
                <div className="flex gap-2">
                  <input type="time" className="border rounded-lg p-3 w-32" value={hours.weekday.open}
                    onChange={e=>setHours(h=>({...h, weekday:{...h.weekday, open:e.target.value}}))} />
                  <span className="self-center">to</span>
                  <input type="time" className="border rounded-lg p-3 w-32" value={hours.weekday.close}
                    onChange={e=>setHours(h=>({...h, weekday:{...h.weekday, close:e.target.value}}))} />
                </div>
              </div>
              <div className="border rounded-lg p-3">
                <div className="font-medium mb-2">Weekend (Sat–Sun)</div>
                <div className="flex gap-2">
                  <input type="time" className="border rounded-lg p-3 w-32" value={hours.weekend.open}
                    onChange={e=>setHours(h=>({...h, weekend:{...h.weekend, open:e.target.value}}))} />
                  <span className="self-center">to</span>
                  <input type="time" className="border rounded-lg p-3 w-32" value={hours.weekend.close}
                    onChange={e=>setHours(h=>({...h, weekend:{...h.weekend, close:e.target.value}}))} />
                </div>
              </div>
            </div>
            <div className="mt-1 flex gap-2 items-center">
              <button onClick={saveHours} className="bg-black text-white px-4 py-2 rounded-lg active:scale-[.98]">Save</button>
        ...
        <details open className="bg-white rounded-xl shadow">
          ...
          <div className="p-4 pt-0 space-y-3">
            <div className="grid sm:grid-cols-2 gap-3">
              <input className="border rounded-lg p-3" placeholder="Customer name" value={qName} onChange={e=>setQName(e.target.value)} />
              <input className="border rounded-lg p-3" placeholder="Phone" inputMode="tel" value={qPhone} onChange={e=>setQPhone(e.target.value)} />
              <input className="border rounded-lg p-3 sm:col-span-2" type="email" placeholder="Email" value={qEmail} onChange={e=>setQEmail(e.target.value)} />

              <select className="border rounded-lg p-3 sm:col-span-2" value={qServiceId} onChange={e=>setQServiceId(e.target.value)}>
                <optgroup label="Men">
                  {SERVICES.filter(s=>s.category==="men").map(s=>(
                    <option key={s.id} value={s.id}>{s.id} — {s.name} ({s.minutes}m)</option>
                  ))}
                </optgroup>
                <optgroup label="Women">
                  {SERVICES.filter(s=>s.category==="women").map(s=>(
                    <option key={s.id} value={s.id}>{s.id} — {s.name} ({s.minutes}m)</option>
                  ))}
                </optgroup>
                <optgroup label="Aesthetic">
                  {SERVICES.filter(s=>s.category==="aesthetic").map(s=>(
                    <option key={s.id} value={s.id}>{s.id} — {s.name} ({s.minutes}m)</option>
                  ))}
                </optgroup>
              </select>

              <input type="date" className="border rounded-lg p-3" value={qDate} onChange={e=>setQDate(e.target.value)} />
              <select className="border rounded-lg p-3" value={qTime} onChange={e=>setQTime(e.target.value)}>
            ...
            <div className="flex gap-2 items-center">
              <button onClick={createBooking} className="bg-black text-white px-4 py-2 rounded-lg active:scale-[.98]">
                Create booking
              </button>
